...
class Logger {
     ...
     */
    constructor(logTimestamps, color = chalk.whiteBright) {
    ...
    logWarn(message, header = 'WARN') {
        return console.log(this.timestamp + chalk.yellow(`[${header}] ` + message));
    }
    ...
    logError(message, header = 'ERROR') {
        return console.log(this.timestamp + chalk.red(`[${header}] ` + message));
    }
